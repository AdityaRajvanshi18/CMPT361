...
bool intersect_chessboard(Vector u, Point o, Point *hit){
  ...
  if(normal_dot_ray == 0) return false; 
...
RGB_float trace(Vector ray, Point o, int iteration) {
  ...
  if(nearest != NULL){
    ...
    if((reflections_on) && (iteration < step_max)){
      ...
      reflection_col = trace(reflection, *hit, iteration);
      reflection_col = clr_scale(reflection_col, nearest->reflectance); 
    ...
    if ((refractions_on) && (iteration < step_max))
    {
      ...
      refraction_col = trace(transmission, *hit, iteration);
  ...
  if(chessboard_on){    
    ...
    if(isBoardHit){
      ...
      if((reflections_on) && (iteration < step_max)){
        ...
        reflection_col = trace(reflection, *chessboard_hit, iteration);
...
void render() {
  int i, j;
  float x_grid_size = image_width / float(winWidth);
  float y_grid_size = image_height / float(winHeight);
  ...
  if(loopertest){
    for(int i = 0; i < 5; i++){
          ...
          frame[r1 + winHeight/2][r2+winWidth/2][0] = GLfloat(ret_color.r);
          frame[r1 + winHeight/2][r2+winWidth/2][1] = GLfloat(ret_color.g);
          frame[r1 + winHeight/2][r2+winWidth/2][2] = GLfloat(ret_color.b);
        }
  }  
}
